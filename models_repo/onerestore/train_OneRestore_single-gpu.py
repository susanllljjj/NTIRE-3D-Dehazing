...
from utils.utils import print_args, load_restore_ckpt, load_embedder_ckpt, adjust_learning_rate, data_process, tensor_metric, load_excel, save_checkpoint
...
def main(args):
    ...
    restorer = load_restore_ckpt(device, freeze_model=False, ckpt_name=args.restore_model_path)
    optimizer = torch.optim.Adam(restorer.parameters(), lr=args.lr)
    cur_epoch = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--degr-type", type=list, default=['clear', 'haze'], help='degradation type')
