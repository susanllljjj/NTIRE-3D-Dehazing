...
## 🌟 系统架构图
1. **预处理 (IHDCP)**: 基于 Matlab 的物理先验去雾，去除首层浓雾。
...
- **下载链接**: [此处填写你的云盘链接]
- **放置路径**:
  - `weights/final_unified_weights.pth` (集成权重包)
...
### 第一步：Matlab 预处理
运行 `matlab_scripts/IHDCP.m` 对原始图像进行首轮处理。

### 第二步：3D 渲染与自动去雾
进入 3D-UIR 目录并运行渲染脚本：
```bash
cd 3D-UIR
# 运行特定场景的渲染（渲染完成后会自动通过 main_system.py 触发去雾）
